// Initialize a Paystack transaction for a PCI DSS request
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: reqRow, error: reqErr } = await supabase
      .from("pci_dss_requests")
      .select("id, user_id, amount_kobo, currency, public_id, email")
      .eq("id", request_id)
      .single();
    ...
    const psRes = await fetch("https://api.paystack.co/transaction/initialize", {
      ...
      body: JSON.stringify({
        email: reqRow.email || user.email,
        amount: reqRow.amount_kobo,
        currency: reqRow.currency || "NGN",
        reference,
        callback_url,
        metadata: { pci_request_id: reqRow.id, user_id: user.id, public_id: reqRow.public_id, source_type: "pci" },
      }),
    });
